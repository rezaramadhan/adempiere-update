...
public final class InventoryUtil
{
	...
	public static MProduct getCreateProduct(MMDocument doc)
	{
		return getCreateProduct(doc.ProductValue,
				doc.scenario.MMPolicy,
				doc.scenario.CostingLevel,
				doc.scenario.CostingMethod);
	}
	...
	public static MOrder createOrder(MMDocument doc, String trxName)
	{
		...
		line.setQty(doc.Qty);
		line.setPrice(doc.Price); //anca_bradau 
		line.saveEx();
		//
		doc.document = order;
...
//		po.saveEx();
	...
	private static void checkLineCosts(MMDocument doc, PO line, boolean isSOTrx)
	{
		if (doc.Cost == null || doc.Cost.signum() == 0)
			return ;
		
		BigDecimal lineQty = null;
		BigDecimal costsActual = getLineCosts(line, lineQty, isSOTrx);
		if (costsActual.compareTo(doc.Cost) != 0)
		{
			throw new RuntimeException("Costs Not Match"
					+" - target="+doc.Cost
					+", actual="+costsActual
					+", doc="+doc);
		}
	...
	private static BigDecimal getLineCosts(PO line, BigDecimal lineQty, boolean isSOTrx)
	{
		...
		String sql = "SELECT"
					...
					+" FROM "+MCostDetail.Table_Name
					+" WHERE "
						+line.get_TableName()+"_ID=?";
